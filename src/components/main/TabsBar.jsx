...
export default function TabsBar() {
    const { t, domainAssetsConfig, defaultAssetsConfig, assetUrl, selectedDomain } = useApp();
    const { lang } = useI18n();

    // ── effective config & pack detection (domain → default) ─────────────────────
    const effectiveCfg = createMemo(() => {
        const cfg = domainAssetsConfig?.();
        if (cfg) return cfg;
        const dflt = typeof defaultAssetsConfig === "function" ? defaultAssetsConfig() : null;
        return dflt;
    });
    const usingDefaultPack = createMemo(() => !domainAssetsConfig?.());
    ...
    // ── locate tabs.yaml (fallback to default pack) ──────────────────────────────
    const tabsPath = createMemo(() => {
        const cfg = effectiveCfg();
        const rel = cfg?.modules?.tabs;
        const fallback = usingDefaultPack() ? "modules/tabs.yaml" : null;
        return rel || fallback;
    });

    const resolveUrl = (relPath) => {
        const clean = String(relPath).replace(/^\/+/, "");
        if (usingDefaultPack()) return `/domain_default/${clean}`;
        try {
            const u = assetUrl ? assetUrl(relPath) : undefined;
    ...
    const [tabsRaw] = createResource(
        ...
        async (relPath) => {
            ...
            if (!res.ok) throw new Error(`tabs.yaml fetch failed: ${res.status}`);
