...
SAVVA UI — это одностраничное приложение на SolidJS, которое предоставляет:
...
UI автоматически получает адреса смарт‑контрактов блокчейна с бэкенд‑эндпойнта `/info`, который читает их из контракта Config.

В конфигурации UI не требуется прописывать адреса контрактов вручную.
...
Папка сборки `dist/` содержит статические файлы, которые может обслуживать любой веб‑сервер.

#### Использование Nginx (рекомендуется)

SAVVA требует полноценной конфигурации Nginx, которая обрабатывает:
- Обслуживание статических файлов UI
- Прокси для бэкенд‑API по пути `/api`
- Пререндеринг для SEO‑ботов
- Динамический эндпойнт конфигурации
- Поддержку WebSocket

**Скачайте полный шаблон конфигурации Nginx:**
...
**Ключевые возможности, включённые в пример:**
1. Перенаправление с HTTP на HTTPS
2. Настройка SSL/TLS (Cloudflare Origin Certificates или Let's Encrypt)
3. Эндпойнт `/default_connect.yaml` — предоставляет UI URL бэкенда и IPFS‑шлюза
4. Пререндеринг для ботов — серверный рендеринг для поисковых систем и соцсетей
5. Прокси `/api` — перенаправляет запросы к бэкенду на порт 7000
6. Поддержка WebSocket — для функций в реальном времени
7. Обслуживание статических файлов с маршрутизацией SPA
8. Умное кеширование — `index.html` никогда не кешируется, ассеты кешируются на 1 год
...
**Развёрнуть файлы и включить сайт:**
...
### Вариант C: Развёртывание через Docker
...
- Убедитесь в правильной настройке CORS на бэкенде
- Проверьте консоль браузера на наличие ошибок
- Проверьте здоровье бэкенда: `curl https://api.yourdomain.com/health`
