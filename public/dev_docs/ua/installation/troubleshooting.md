...
**Симптом**: Не вдається завантажити/отримати з IPFS
...
**Симптом**: Бекенд споживає надто багато пам'яті

**Рішення**:
- Переглянути налаштування пулу з'єднань
- Перевірити журнали на витоки пам'яті
- Періодично перезапускати сервіс
- Розглянути можливість збільшення ОЗП сервера
...
### Не вдається підключитися до API

**Симптом**: Інтерфейс не може підключитися до бекенда
...
**Симптом**: Не вдається підключити MetaMask або інші гаманці

**Рішення**:
- **Переконайтеся в наявності HTTPS**: Web3 вимагає захищеного з'єднання
- **Перевірте розширення гаманця**: Воно встановлене та розблоковане?
...
**Симптом**: HTTPS не працює або є попередження про сертифікат
...
### Проблеми з розв'язанням DNS

**Симптом**: Домен не розв'язується
...
**Симптом**: Не вдається отримати доступ до сервісів віддалено
...
**Симптом**: Інтерфейсу потрібен тривалий час для завантаження

**Рішення**:
- Увімкнути Gzip-компресію в Nginx
- Налаштувати CDN (Cloudflare тощо)
- Перевірити час відповіді бекенда
- Оптимізувати запити до бази даних
- Увімкнути кешування в браузері

### Високе завантаження CPU

**Симптом**: Завантаження CPU сервера 100%
...
- У користувача немає прав
- Перевірте привілеї в базі даних

### "CORS policy" errors
- CORS бекенда не налаштований
- Неправильний origin у allowed_origins
- Не проходить preflight-запит

### "network error" in UI
- Бекенд недоступний
- Неправильний URL API у конфігурації UI
...
*Цей посібник з усунення неполадок буде доповнюватися в міру документування нових проблем.*